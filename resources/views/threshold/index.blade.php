<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 xl:px-16 xl:ml-64 space-y-14">
        ...
        <!-- Tombol untuk menambah threshold -->
        <div class="flex mt-16 items-center justify-between">
            <h2 class="font-semibold text-xl">Threshold</h2>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-center">Minimum Stock</th>
                ...
                <tbody>
                    @foreach ($thresholds as $threshold)
                        <tr
                            ...
                            <td class="px-6 py-4 text-center">{{ $threshold->minimum_stock }}</td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('threshold.edit', $threshold) }}"
                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>

                                <!-- Button for Delete -->
                                <button type="button"
                                    class="font-medium text-red-600 dark:text-red-500 hover:underline"
                                    onclick="confirmDelete('{{ $threshold->product->name }}', '{{ route('threshold.destroy', $threshold) }}')">
                                    Delete
                                </button>
                            </td>
                        </tr>
...
<script>
    function confirmDelete(name, url) {
        Swal.fire({
            title: 'Are you sure?',
            text: `You are about to delete the threshold: ${name}. This action cannot be undone.`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                fetch(url, {
                    ...
                    .then(async (response) => {
                        if (!response.ok) {
                            ...
                            throw new Error(errorData.message ||
                                'Failed to delete the threshold.');
                        }
                    ...
                    .then((data) => {
                        ...
                        Swal.fire(
                            'Deleted!',
                            `${name} from threshold has been deleted.`,
                            'success'
                        ).then(() => {
