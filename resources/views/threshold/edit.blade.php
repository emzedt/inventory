<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 xl:px-16 xl:ml-64 space-y-6">
        <div class="flex mt-16 items-center justify-between">
            <h2 class="font-semibold text-xl">Edit Threshold</h2>
        </div>

        <div class="mt-4">
            <form enctype="multipart/form-data" method="POST" action="{{ route('threshold.update', $threshold) }}">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <select id="product" name="product_id"
                        ...
                        @foreach ($products as $product)
                            <option value="{{ $product->id }}"
                                {{ $product->id == $threshold->product_id ? 'selected' : '' }}>
                                {{ $product->name }}
                            </option>
                        @endforeach
                    </select>
